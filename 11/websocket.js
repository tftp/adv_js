...
function addChannel(){
  ...
  let string = `{"command":"login", "channel":"${channel}"}`;
  if (name.length > 0)  string = `{"command":"login", "channel":"${channel}", "name":"${name}"}`;
  socket.send(string);
};

function removeChannel(){
  let channel = inputChannel.value
  let string = `{"command":"logout", "channel":"${channel}"}`
  socket.send(string);
};
...
function sendCommand(){
  ...
  let string = `{"command":"sendMessage", "channel":"${channel}", "message":"${message}"}`
  inputSend.value = '';
  socket.send(string);
};

function exitChat(){
  let string = `{"command":"exitChat"}`
  socket.send(string);
};
